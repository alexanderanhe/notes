...
import type { Route } from "./+types/api.vault-items.$itemId.pinned";
...
export async function action({ params, request }: Route.ActionArgs) {
  ...
  if (!parsed.success) throw new Response("Invalid pinned value.", { status: 400 });
  const item = await setVaultItemFlag(user._id, params.itemId, "pinned", parsed.data.pinned);
